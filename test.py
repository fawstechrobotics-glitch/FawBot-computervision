...
UI & Detection Enhancements:
  ...
  - Full-screen display mode active by default (press 'F' to toggle)
...
class RobotAgent:
    ...
    def send(self, cmd_str, force=False):
        ...
        try:
            ...
            self.sock.sendto(cmd_str.encode("utf-8"), addr)
...
class MultiRobotFleetController:
    def __init__(self, cam_idx=0, fullscreen=True):
    ...
    def _on_mouse(self, event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            self.unassigned_points.append((x, y))
    ...
    def run(self):
        try:
            while True:
                ...
                # Safe post-render fullscreen initialization for macOS
                if self.is_fullscreen and not self._fullscreen_applied:
